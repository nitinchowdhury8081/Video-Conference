// Create gmeet client
var client = gmeetRTC.createClient({ mode: "rtc", codec: "vp8" });
...
async function RTMJoin() { // Create gmeet RTM client
    const clientRTM = gmeetRTM.createInstance($("#appid").val(), { enableLogUpload: false });
    var accountName = $('#accountName').val();
    // Login
    clientRTM.login({ uid: accountName }).then(() => {
        console.log('gmeetRTM client login success. Username: ' + accountName);
        isLoggedIn = true;
        // RTM Channel Join
        var channelName = $('#channel').val();
        channel = clientRTM.createChannel(channelName);
        channel.join().then(() => {
            console.log('gmeetRTM client channel join success.');
            ...
            console.log('gmeetRTM client channel join failed: ', error);
        }).catch(err => {
            console.log('gmeetRTM client login failure: ', err);
        });
    });
    // Logout
